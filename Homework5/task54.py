...
def scrabble(word):
    ...
    # Занулим количество очков points
    points = 0

    # Зададим словарь, где буквы - ключи, очки - значения
    dict_score = {'а': 1, 'в': 1, 'е': 1, 'ё': 1, 'и': 1, 'н': 1, 'о': 1, 'р': 1, 'с': 1, 'т': 1,
    ...
    for item in range(len(word)):
        ...
        # Проверяем для каждого элемента строки word есть ли он среди ключей словаря,
        # если есть, то к количеству очков прибавляем значение соотв ключа,
        # если нет, то ничего не делаем
        if word[item] in dict_score.keys():
            points += dict_score.get(word[item])
        else:
            pass

...
def repeats(our_str):
    ...
    # Объявляем пустую строку new_str
    new_str = str()

    for item in range(len(our_str)):

        # Плюсуем каждый item элемент строки our_str к новой строке new_str, добавляем '_' и добавляем
        # сколько раз этот item элемент уже встречался
        new_str = new_str + our_str[item] + "_" + str(our_str[:item+1:].count(our_str[item]))

...
class Segment:
    ...
    # Инициализируем класс Segment
    def __init__(self, begin, end):
        self.begin = begin
        self.end = end

    # Метод, который возвращает длину с точностью до 2 знаков после запятой
    def length(self):
        x1, y1 = self.begin
        x2, y2 = self.end
        return round(math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2), 2)

    # Метод, который возвращает True, если отрезок пересекает ось абцисс, иначе False

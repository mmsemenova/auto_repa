...
class PublicTransport:
    def __init__(self, brand, _engine_power, year, color, max_speed):
        # public Марка транспорта
        self.brand = brand
        # Protected Мощность двигателя
        self._engine_power = _engine_power
        # public Год выпуска
        self.year = year
        # public Цвет
        self.color = color
        # public Максимальная скорость
        self.max_speed = max_speed

    # Cвойство info, которое выводить на печать информацию о: марке, цвете, годе выпуска и мощности двигателя
    @property
    def info(self):
...
class Bus(PublicTransport):
    ...
    def __init__(self, brand, _engine_power, year, color, max_speed, passengers, __park, _fare):
        PublicTransport.__init__(self, brand, _engine_power, year, color, max_speed)
        # public кол-во пассажиров
        self.passengers = passengers
        # private Парк приписки автобуса
        self.__park = __park
        # protected Cтоимость проезда
        self._fare = _fare

    # Cвойство park, которое будет возвращать значение приватного атрибута park
    @property
    def park(self):
        return self.__park

    # Сеттер свойства park, которое возвращает значение park,
    # перед этим проверив, входит ли оно в диапазон от 1000 до 9999
    @park.setter
    def park(self, park):
...
class Tram (PublicTransport):
    def __init__(self, brand, _engine_power, year, color, max_speed, __route, path, _fare):
        PublicTransport.__init__(self, brand, _engine_power, year, color, max_speed)
        # private маршрут трамвая
        self.__route = __route
        # public длина маршрута
        self.path = path
        # protected cтоимость проезда
        self._fare = _fare

    # Cвойство how_long, которое вычисляет время прохождения маршрута по формуле max_speed/(4*path)
    @property
    def how_long(self):
        return round(float(self.max_speed) / (4 * float(self.path)), 2)
